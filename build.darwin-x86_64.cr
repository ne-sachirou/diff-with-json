...
cc_cmd = IO.pipe do |r, w|
  puts "crystal build --release -o bin/diff-with-json-darwin-x86_64 --cross-compile --target #{clang_target} bin/diff-with-json.cr"
  Process.run "crystal", ["build", "--release", "-o", "bin/diff-with-json-darwin-x86_64", "--cross-compile", "--target", clang_target, "bin/diff-with-json.cr"], output: w
  w.close
  r.gets_to_end
end
